;(function ( $, window, document, undefined ) {
 	...
 	/*
 		Note:
 		Provide some instructions on how to use the menu with a keyboard and add them to the default options.
	*/
	...
	Plugin.prototype.init = function () {
		...
		plugin.menuitems = $elem.find('li') // setup menuitems
			...
			.each(function(i, el) {
				...
				/*
					Note:
					For each of the menu items that has a submenu.
				*/
				$me.has('ul')
				    .attr({ // setup submenus
					    'aria-haspopup': true, // announce the presence of the submenu
					    'aria-expanded': false // default state to “collapsed”
				    })
					.addClass('expandable');
			});
		...
		/* Add keyboard access back to menu items */
		...
		plugin.menuitems // setup event handlers
			...
			.on("keydown", {'plugin': plugin}, plugin.onKeyDown); // reference the keyboard access class
	...
	Plugin.prototype.collapseAll = function(plugin, $elem) {
		...
		/*
			Note:
			Collapse any open menus by reversing all attributes defining the element as open.
		*/
